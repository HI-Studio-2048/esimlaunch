...
import { Eye, EyeOff, Mail, Lock, Globe, Shield, Loader2 } from "lucide-react";
...
const Login = () => {
  ...
  const handleSocialLogin = async (e: React.MouseEvent, provider: 'google' | 'apple') => {
    ...
    if (provider === 'apple') {
      toast({ title: "Coming Soon", description: "Apple login will be available soon." });
      return;
    }
    ...
    // Clear the explicit logout flag — user is actively trying to log in
    ...
    try {
      ...
      toast({
        title: "Login failed",
        description: err?.message || "Failed to start Google sign-in. Please try again.",
        variant: "destructive",
      });
    }
  };
