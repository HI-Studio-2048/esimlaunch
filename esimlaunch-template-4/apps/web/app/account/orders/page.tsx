  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:py-16">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-void">Order History</h1>
        <Link
          href="/account"
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          <p className="text-dim">You haven&apos;t placed any orders yet.</p>
          <Link
            href="/"
        ...
        <div className="overflow-x-auto rounded-card border border-rule bg-surface shadow-card">
          <table className="min-w-full divide-y divide-rule">
            <thead>
              <tr>
                <th className="px-5 py-4 text-left text-xs font-medium uppercase tracking-wide text-dim">
                  Order
                </th>
                <th className="px-5 py-4 text-left text-xs font-medium uppercase tracking-wide text-dim">
                  Plan
                ...
                <th className="px-5 py-4 text-right text-xs font-medium uppercase tracking-wide text-dim">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-rule">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-canvas">
                  <td className="px-5 py-4 text-sm font-mono text-dim">{order.id.slice(0, 8)}</td>
                  <td className="px-5 py-4 text-sm text-void">
                    {order.planName ?? order.planId}
                  </td>
                  <td className="px-5 py-4 text-sm text-void">{formatAmount(order)}</td>
                  <td className="px-5 py-4">
                    <span className="badge-gray">
                      {order.status}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-sm text-dim">{formatDate(order.createdAt)}</td>
...
function ReceiptButton({ orderId, authFetchBlob }: { orderId: string; authFetchBlob: (path: string, options?: RequestInit) => Promise<Blob> }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="text-sm font-medium text-link hover:underline disabled:opacity-50"
    >
      {loading ? '…' : 'Receipt'}
    </button>
  );
