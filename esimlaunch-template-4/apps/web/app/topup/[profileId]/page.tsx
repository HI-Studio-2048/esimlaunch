...
export default function TopUpPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:py-12">
      ...
      <h1 className="mb-8 text-2xl font-bold text-[#1d1d1f]">Top Up Your eSIM</h1>

      {error && (
        <div className="mb-6 rounded-[18px] border border-red-100 bg-red-50 p-4 text-red-700">
          {error}
        </div>
      )}

      {plans.length === 0 && !error ? (
        <div className="rounded-[18px] border border-dashed border-[#d2d2d7] bg-[#f5f5f7]/50 py-16 text-center">
          <p className="text-[#6e6e73]">No top-up plans available for this eSIM.</p>
        </div>
      ) : (
        <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3">
          {plans.map((plan) => (
            <div
              ...
              className="rounded-[18px] border border-[#d2d2d7] bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.06)] transition-all duration-200 hover:border-[#adadb8] hover:shadow-[0_4px_20px_rgba(0,0,0,0.10)]"
            >
              <p className="text-xl font-bold text-[#1d1d1f]">{formatVolume(plan.volume)}</p>
              <p className="mt-1 text-sm text-[#6e6e73]">
                {plan.duration} {plan.durationUnit}
                {plan.duration > 1 ? 's' : ''}
              </p>
              <p className="mt-4 text-2xl font-bold text-[#0066cc]">
                {formatProviderPrice(plan.price)}
              </p>
              <button
                onClick={() => handleSelect(plan)}
                disabled={selecting === plan.packageCode}
                className="mt-4 w-full rounded-full bg-[#1d1d1f] py-3 text-sm font-semibold text-white transition hover:bg-[#2d2d2f] disabled:cursor-not-allowed disabled:opacity-40"
              >
