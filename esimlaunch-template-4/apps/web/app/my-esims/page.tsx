    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-10 sm:py-12">
        ...
        <div className="flex flex-col gap-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="rounded-card border border-rule bg-surface p-6">
              ...
              <div className="mt-4 flex gap-2">
                <Skeleton className="h-9 w-24 rounded-btn-sq" />
                <Skeleton className="h-9 w-20 rounded-btn-sq" />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:py-12">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-bold text-void sm:text-3xl">My eSIMs</h1>
        <Link
          href="/"
        ...
        <>
          ...
          <div className="mb-6 flex flex-wrap gap-4">
            ...
            <input
              ...
              className="input"
            />
            <label className="flex items-center gap-2 text-sm text-void">
              <input
                type="checkbox"
          ...
          {selectedIds.size > 0 && (
            <div className="mb-4 flex items-center gap-3 rounded-card border border-rule bg-canvas px-4 py-3">
              <span className="text-sm font-medium text-void">{selectedIds.size} selected</span>
              <BulkDownloadQr
...
function EsimCard({
  ...
  return (
    <div className={`rounded-card border p-6 shadow-card bg-surface ${selected ? 'border-void/30 ring-2 ring-void/10' : 'border-rule'}`}>
        ...
        <div className="flex flex-1 min-w-0 items-start gap-3">
          {onToggleSelect && (
            <input
              ...
              className="mt-1 h-4 w-4 shrink-0 rounded border-rule"
            />
          )}
          <div>
            <Link
              href={profile.iccid ? `/my-esims/${profile.iccid}` : '#'}
              className="font-semibold text-void hover:text-link hover:underline transition-colors"
            >
              {profile.order?.planName ?? 'eSIM Plan'}
            </Link>
            {profile.iccid && (
              <p className="mt-1 text-xs text-dim">ICCID: {profile.iccid}</p>
            )}
      ...
      {totalMb > 0 && (
        <div className="mt-5">
          ...
          <div className="h-2 overflow-hidden rounded-full bg-canvas">
            <div
              className="h-full rounded-full bg-void transition-all duration-500"
              style={{ width: `${pctUsed}%` }}
            />
          </div>
        </div>
      ...
      {profile.expiredTime && (
        <div className="mt-3 text-xs">
          ...
          <ExpiryCountdown
            ...
            iccid={profile.iccid}
            getToken={getToken}
            className="font-medium text-void"
          />
        </div>
      )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {profile.esimStatus === 'IN_USE' && profile.iccid && (
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-btn-sq border border-rule bg-canvas px-4 py-2.5 text-sm font-medium text-void no-underline transition hover:bg-surface"
          >
            + Top Up Plan
          </Link>
        )}
...
function CopyActivationCode({ ac }: { ac: string }) {
  ...
  return (
    <div className="mt-4">
      <p className="mb-1 text-xs font-medium text-dim">Activation Code</p>
  ...
  return (
    <>
      <button
        ...
        className="flex-shrink-0 rounded-card border border-rule bg-surface p-2 shadow-card transition hover:shadow-card-hover"
      ...
      {showModal && (
        ...
        >
          <div
            className="relative rounded-card-lg bg-surface p-6 shadow-card-hover"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setShowModal(false)}
              className="absolute right-3 top-3 rounded-full p-1 text-dim hover:bg-canvas hover:text-void transition"
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <p className="mb-3 text-center text-sm font-medium text-void">{planName}</p>
            <Image
              src={qrCodeUrl}
              alt="eSIM QR Code"
              width={280}
