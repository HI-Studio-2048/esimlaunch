  ...
  if (!isLoaded || !isSignedIn) {
    return (
      <div className="mx-auto max-w-md px-4 py-16">
        <p className="text-[#6e6e73]">Please sign in to view this ticket.</p>
  ...
  if (error || !ticket) {
    return (
      <div className="mx-auto max-w-md px-4 py-16">
        ...
        <Link href="/support/tickets" className="mt-4 inline-block text-[#0066cc] hover:underline">
          ← Back to tickets
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:py-24">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#1d1d1f]">{ticket.subject}</h1>
        <span className="rounded-full bg-[#f5f5f7] px-3 py-1 text-sm font-medium text-[#6e6e73]">
          {ticket.status}
        </span>
      </div>

      <div className="card p-6">
        <p className="text-[#1d1d1f]">{ticket.body}</p>
        <p className="mt-4 text-xs text-[#adadb8]">
      ...
      {ticket.replies.length > 0 && (
        <div className="mt-8 space-y-4">
          <h2 className="font-semibold text-[#1d1d1f]">Replies</h2>
          {ticket.replies.map((reply) => (
            ...
            >
              <p className="text-[#1d1d1f]">{reply.body}</p>
      ...
      <form onSubmit={handleReply} className="mt-8">
        <label className="mb-2 block font-medium text-[#1d1d1f]">Add a reply</label>
        <textarea
          value={replyBody}
        ...
        <button
          ...
          className="btn-primary mt-4 rounded-full px-6 py-2.5 font-medium disabled:opacity-50"
        >
      ...
      <div className="mt-8">
        <Link href="/support/tickets" className="text-sm font-medium text-[#0066cc] hover:underline">
          ← Back to tickets
        </Link>
      </div>
    </div>
