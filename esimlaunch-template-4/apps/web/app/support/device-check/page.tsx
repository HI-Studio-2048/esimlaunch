  ...
  return (
    <div className="min-h-screen bg-surface">
      <div className="mx-auto max-w-4xl px-4 py-10">
        <Link
          href="/"
          className="mb-6 inline-flex items-center gap-2 text-dim transition-colors hover:text-void"
        >
        ...
        <div className="mb-8 text-center">
          <h1 className="mb-2 text-4xl font-bold text-void">
        ...
        <Card className="mb-6 border-rule bg-surface">
          <CardHeader>
            <CardTitle className="text-void">Search Your Device</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="relative" ref={suggestionsRef}>
              <div className="relative">
                ...
                <Input
                  ...
                  className="border-rule bg-surface pl-10 text-void placeholder:text-faint"
                />
              </div>
              {showSuggestions && suggestions.length > 0 && (
                <div className="absolute z-50 mt-1 max-h-60 w-full overflow-y-auto rounded-card border border-rule bg-surface shadow-card-hover">
                  {suggestions.map((model) => (
                    <button
                      key={model}
                      type="button"
                      onClick={() => handleSuggestionClick(model)}
                      className="w-full px-4 py-2 text-left text-void transition-colors hover:bg-canvas"
                    >
                      {model}
                    </button>
                  ))}
            ...
            <div className="relative">
              ...
              <select
                ...
                className="h-10 w-full rounded-btn-sq border border-rule bg-surface pl-10 pr-3 text-void focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-void"
              >
        ...
        {result && (
          <Card className="border-rule bg-surface">
            ...
            <CardContent className="space-y-4">
              <div>
                <p className="text-sm text-faint">Device</p>
                <p className="text-lg font-semibold text-void">
              ...
              {result.notes && result.notes.length > 0 && (
                <div>
                  <p className="mb-2 text-sm text-faint">Important Notes</p>
                  <div className="space-y-2">
                    {result.notes.map((note, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-2 rounded-btn-sq border border-amber-200 bg-amber-50 p-3"
                      >
              ...
              {selectedCountry &&
                result.regionalNotes?.[selectedCountry.toUpperCase()] && (
                  <div>
                    ...
                    <div className="rounded-btn-sq border border-amber-300 bg-amber-50 p-3">
                ...
                {result.supported ? (
                  <>
                    <Link href="/">
                      <Button
                        variant="outline"
                        className="border-rule text-dim hover:bg-canvas"
                      >
                        Browse Plans
                    ...
                    <Link href="/support">
                      <Button
                        variant="secondary"
                        className="bg-canvas text-dim hover:bg-rule/30"
                      >
