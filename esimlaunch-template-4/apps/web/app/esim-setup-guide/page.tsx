...
export default function EsimSetupGuidePage() {
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:py-24">
      <h1 className="text-3xl font-bold text-void">eSIM Setup Guide</h1>
      ...
      <div className="mt-12 space-y-10">
        {STEPS.map((step, i) => (
          <div key={i} className="flex gap-6">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-canvas border border-rule text-void font-semibold">
              {i + 1}
            </div>
            <div>
              <h2 className="text-xl font-semibold text-void">{step.title}</h2>
              <p className="mt-2 text-dim">{step.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Things to know */}
      <div className="mt-16 rounded-card border border-rule bg-canvas p-6">
        <h2 className="text-lg font-semibold text-void">Things to know about eSIM</h2>
