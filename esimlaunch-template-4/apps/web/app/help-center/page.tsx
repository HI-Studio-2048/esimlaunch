...
export default function HelpCenterPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:py-24">
      <h1 className="text-3xl font-bold text-void">Help Center</h1>
      ...
      <div className="mt-12 space-y-10">
        {byCategory.length === 0 ? (
          <p className="text-dim">No articles match your search.</p>
        ) : (
          byCategory.map(({ category, articles }) => (
            <div key={category}>
              <h2 className="text-lg font-semibold text-void">{category}</h2>
                ...
                {articles.map((article) => (
                  ...
                  >
                    <button
                      ...
                      className="flex w-full items-center justify-between px-5 py-4 text-left hover:bg-canvas rounded-card"
                    >
                      <div>
                        <span className="font-medium text-void">{article.title}</span>
                        <p className="mt-1 text-sm text-dim">{article.shortDesc}</p>
                      </div>
                      <span
                        className={`ml-4 shrink-0 text-xs text-faint transition-transform ${expandedId === article.id ? 'rotate-180' : ''}`}
                      >
                        ▼
                      </span>
                    </button>
