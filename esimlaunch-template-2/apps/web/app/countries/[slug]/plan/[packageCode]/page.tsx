  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <p className="mt-4 text-ink-muted">Loading plan…</p>
      </div>
    );
  }
  ...
  if (error || !plan || !location) {
    return (
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        ...
        <Link
          href={`/countries/${slug}`}
          className="mt-6 inline-block text-teal-400 hover:underline"
        >
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:py-12">
      ...
      <nav className="mb-8 text-sm text-ink-muted">
        ...
        <span className="mx-2">/</span>
        <Link href={`/countries/${slug}`} className="hover:text-ink">
      ...
      <div className="card p-8">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <p className="section-label mb-2">Plan Details</p>
            <h1 className="text-display-md text-ink">{plan.name}</h1>
        ...
        <dl className="space-y-4 border-y border-surface-border py-6">
          <div className="flex justify-between">
            <dt className="text-ink-muted">Data</dt>
            <dd className="font-medium text-ink">{formatVolume(plan.volume)}</dd>
          </div>
          <div className="flex justify-between">
          ...
          {plan.supportTopUpType === 2 && (
            <div className="flex justify-between">
              <dt className="text-ink-muted">Top-up</dt>
