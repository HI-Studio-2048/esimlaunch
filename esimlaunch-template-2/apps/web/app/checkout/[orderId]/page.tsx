...
export default function CheckoutPage() {
      ...
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        <div className="card p-8">
          <p className="text-ink-secondary">
            ...
            <a href="/my-esims" className="font-medium text-teal-400 hover:text-teal-500">
              My eSIMs
            </a>
            .
          </p>
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-12 sm:py-16">
      ...
      <h1 className="mb-8 text-display-md text-ink">Checkout</h1>

      {/* Order summary */}
      <div className="card mb-6 p-6">
        <p className="section-label mb-3">Order Summary</p>
        <p className="font-semibold text-ink">{order.planName ?? order.planId}</p>
      ...
      {/* Promo code */}
      <div className="mb-5">
        <label className="mb-2 block text-sm font-medium text-ink-secondary">Promo code</label>
        <div className="flex gap-2">
          <input
            type="text"
            value={promoCode}
            ...
            placeholder="PROMO"
            className="input flex-1 uppercase disabled:bg-surface-muted"
            disabled={promoApplied}
          />
          {promoApplied ? (
            <button
              onClick={handleRemovePromo}
              className="rounded-btn border border-red-200 px-4 py-2.5 text-sm font-medium text-red-600 transition hover:bg-red-50"
            >
              Remove
            </button>
          ) : (
      ...
      {message && (
        <p className="mb-4 rounded-btn bg-teal-50 px-4 py-3 text-sm text-teal-700">
          {message}
        </p>
      )}
      ...
      {canPayStoreCredit && (
        <button
          ...
          disabled={paying}
          className="mb-4 w-full rounded-btn border-2 border-teal-200 bg-teal-50 py-3.5 font-semibold text-teal-700 transition hover:bg-teal-100 disabled:cursor-not-allowed disabled:opacity-50"
        >
