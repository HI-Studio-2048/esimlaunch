    ...
    <div className="mx-auto max-w-2xl px-4 py-16 sm:py-24">
      ...
      <div className="mt-12 space-y-3">
        {items.map((item, i) => (
          <div
            key={i}
            className="rounded-card border border-surface-border bg-white shadow-sm"
          >
            <button
              onClick={() => setOpenIndex(openIndex === i ? null : i)}
              className="flex w-full items-center justify-between px-5 py-4 text-left font-medium text-ink hover:bg-surface-soft"
            >
              {item.question}
            ...
            {openIndex === i && (
              <div className="border-t border-surface-border px-5 py-4 text-ink-muted">
                {item.answer}
              </div>
            )}
          </div>
