...
function getStatusDisplay(esimStatus: string | undefined): {
  ...
  if (!esimStatus) return { label: 'Pending', color: 'bg-slate-100 text-slate-600' };
  const statusMap: Record<string, { label: string; color: string }> = {
    GOT_RESOURCE: { label: 'Ready', color: 'bg-green-100 text-green-700' },
    IN_USE: { label: 'Active', color: 'bg-blue-100 text-blue-700' },
    EXPIRED: { label: 'Expired', color: 'bg-red-100 text-red-700' },
    SUSPENDED: { label: 'Suspended', color: 'bg-amber-100 text-amber-700' },
  };
  return statusMap[esimStatus] ?? { label: esimStatus, color: 'bg-slate-100 text-slate-600' };
}
  ...
  if (!isLoaded || loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <p className="mt-4 text-slate-500">Loading eSIM details…</p>
      </div>
    );
  }
  ...
  if (!profile) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        <h1 className="text-2xl font-bold text-slate-900">eSIM not found</h1>
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-slate-900">eSIM Details</h1>
      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="flex items-center gap-3">
            <span
              className={`rounded-full px-3 py-1 text-sm font-medium ${statusDisplay.color}`}
            >
              {statusDisplay.label}
            </span>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="rounded-xl border border-slate-200 bg-white p-5 text-center">
            <HardDrive className="mx-auto mb-2 h-5 w-5 text-violet-500" />
            <p className="text-sm text-slate-500">Data Balance</p>
            <p className="text-xl font-bold text-slate-900">{sizeGB}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-5 text-center">
            <Calendar className="mx-auto mb-2 h-5 w-5 text-violet-500" />
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 text-center">
            ...
            <p className="text-xl font-bold text-slate-900">{statusDisplay.label}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-5 text-center">
            <p className="text-sm text-slate-500">ICCID</p>
